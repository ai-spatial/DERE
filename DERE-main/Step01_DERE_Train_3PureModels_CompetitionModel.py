import os
from exp.exp_informer import Exp_MultiBranch_AddPure_AddCom_Finetune_Mixdata_ShareStructure
import torch
...
Exp = Exp_MultiBranch_AddPure_AddCom_Finetune_Mixdata_ShareStructure
...
for ii in range(args.itr):
    ...
    exp.train(setting=setting, stage='pure')

    # test             
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting= setting, stage='pure')


    # train
    print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
    exp.train(setting= setting, stage='mix')

    # test             
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting= setting, stage='mix')
