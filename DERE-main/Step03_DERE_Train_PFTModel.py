...
from exp.exp_informer import Exp_ed_allage_pft_prediction_brief
import torch
...
args.seq_len = 28 # [idx : idx+seq_len]: input sequence length of Informer encoder
...
args.pred_len = 28 # [idx+seq_len : idx+seq_len+pred_len]: prediction sequence length; no overlap iwth seq_len
# Informer decoder input: concat[start token series(label_len), zero padding series(pred_len)]

args.enc_in = 136*12 # encoder input size
...
args.checkpoints = './checkpoints_pft_prediction' # location of model checkpoints

Exp = Exp_ed_allage_pft_prediction_brief

for ii in range(args.itr):
    setting = '{}_{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_at{}_fc{}_eb{}_dt{}_mx{}_{}_PFT_{}'.format(args.model, args.data, 
    ...
    exp.train(setting=setting, stage='step1')

    # test             
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting= setting, stage='step1')

    # train
    print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
    exp.train(setting=setting, stage='step2')

    # test             
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting= setting, stage='step2')   
