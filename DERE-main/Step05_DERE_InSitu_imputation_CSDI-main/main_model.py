...
class CSDI_base(nn.Module):
    ...
    def calc_loss(
        ...
        if hasattr(self, "varname"):
            if self.varname == "gpp":
                pivot = -0.72868288
                predicted = F.relu(predicted - pivot) + pivot # Shuo
            elif self.varname == "reco":
                pivot = -0.59285861
                predicted = F.relu(predicted - pivot) + pivot # Shuo
            elif self.varname == "all":
                pivot = -0.72868288
                predicted[ :, 0, :] = F.relu(predicted[ :, 0, :] - pivot) + pivot # Shuo
                pivot = -0.59285861
                predicted[ :, 1, :] = F.relu(predicted[ :, 1, :] - pivot) + pivot # Shuo
        ...
        # Shuo
        if self.varname == "all":
            target_mask = (observed_mask - cond_mask)[ :, :3, : ]
            residual = (noise - predicted)[:, :3, :] * target_mask
            num_eval = target_mask.sum()
            loss1 = (residual ** 2).sum() / (num_eval if num_eval > 0 else 1)
            loss2 = (((predicted[:, 1, :] -predicted[:, 0, :] - predicted[:, 2, :])* target_mask[:, 2, :])**2).mean()
            loss = loss1 + 0.1*loss2
            # Shuo
        else:
            target_mask = (observed_mask - cond_mask)[ :, :1, : ]
            residual = (noise - predicted)[:, :1, :] * target_mask   
            num_eval = target_mask.sum()         
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            ...
            # # Shuo
            # pivot = -0.7287 
            # imputed_samples[:, i] = F.relu(imputed_samples[:, i] - pivot) + pivot
            ...
            if hasattr(self, "varname"):
                if self.varname == "gpp":
                    ...
                    pivot = -0.72868288
                    imputed_samples[ :, i, 0, :] = F.relu(imputed_samples[ :, i, 0, :] - pivot) + pivot # Shuo
                    pivot = -0.59285861
                    imputed_samples[ :, i, 1, :] = F.relu(imputed_samples[ :, i, 1, :] - pivot) + pivot # Shuo

            # imputed_samples[:, i] = torch.relu(current_sample.detach())
...
class CSDI_Forecasting(CSDI_base):
    def __init__(self, config, device, target_dim):
        ...
        self.target_dim_base = target_dim
        self.num_sample_features = self.target_dim_base
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            samples = samples[:, :, :1, :]          # (B, nsample, 1, L)
            observed_data = observed_data[:, :1, :] # (B, 1, L)
            target_mask = target_mask[:, :1, :]     # (B, 1, L)
            observed_mask = observed_mask[:, :1, :] # (B, 1, L)
