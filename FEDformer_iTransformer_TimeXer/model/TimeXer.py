...
# class DataEmbedding_inverted(nn.Module):
#   def __init__(self, c_in, d_model, patch_len=3, dropout=0.1):
#     super(DataEmbedding_inverted, self).__init__()
#     self.patch_len = patch_len
#     self.use_patch = patch_len > 1

#     # Linear projection from patch → d_model
#     self.value_embedding = nn.Linear(c_in * patch_len, d_model)
#     self.dropout = nn.Dropout(p=dropout)
...
#     if self.use_patch:
#       # Unfold into non-overlapping patches of length `patch_len` over time
#       x = x.unfold(dimension=-1, size=self.patch_len, step=self.patch_len)
#       # x: [B, C, Num_Patches, patch_len]
#       B, C, NP, P = x.shape

#       # Reshape to [B, Num_Patches, C * patch_len]
#       x = x.permute(0, 2, 1, 3).reshape(B, NP, C * P)
#     else:
#       # No patching, just permute to [B, T, C]
#       x = x.permute(0, 2, 1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.patch_num = int((configs.label_len + configs.pred_len) // 1)
        self.n_vars = 1 if configs.features == 'MS' else configs.dec_in # dec_in is the number of variables
        # Embedding
    ...
    def forecast_multi(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        en_embed, n_vars = self.en_embedding(x_dec.permute(0, 2, 1))
